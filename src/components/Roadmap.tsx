export default function Roadmap() {
  ...
  return (
    <section id="roadmap" className="py-16 md:py-24 bg-[--color-surface]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:block">
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute top-24 left-0 right-0 h-0.5 bg-[--color-border]"></div>
              ...
              {phases.map((phase, index) => (
                <div key={phase.phase} className="relative">
                  ...
                  <div className={`absolute -top-8 left-1/2 transform -translate-x-1/2 w-4 h-4 rounded-full border-4 border-white ${
                    phase.status === 'current' ? 'bg-[--color-primary]' : 'bg-[--color-border]'
                  }`}></div>

                  <div className="bg-white border border-[--color-border] rounded-lg p-6 shadow-sm h-64">
          ...
          {phases.map((phase, index) => (
            <div key={phase.phase} className="relative">
              <div className="flex items-start gap-4">
                ...
                <div className="bg-white border border-[--color-border] rounded-lg p-4 shadow-sm flex-1">
              ...
              {index < phases.length - 1 && (
                <div className="absolute left-2 top-6 w-0.5 h-16 bg-[--color-border]"></div>
              )}
            </div>
          ))}
        </div>
